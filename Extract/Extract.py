...
# Lead selection (for multi-lead datasets)
LEAD_COLUMN = 'lead_1'  # Which lead to analyze

# Plotting options
PLOT_FULL_SIGNAL = True  # Show complete ECG signal
PLOT_SEGMENT = True  # Show zoomed segment (1/4 of signal)
FIGURE_WIDTH = 12  # Width for full signal plot
FIGURE_HEIGHT = 4  # Height for plots
SEGMENT_WIDTH = 20  # Width for segment plot


# ============================================================================
# ECG FEATURE EXTRACTOR CLASS
# ============================================================================
...
def plot_ecg_segment(ecg_signal, sampling_rate, segment_fraction=4):
    """Plot a segment of ECG signal with detailed grid"""
    cut_ecg_signal = ecg_signal[:len(ecg_signal) // segment_fraction]
    time = np.arange(len(cut_ecg_signal)) / sampling_rate
    
    plt.figure(figsize=(SEGMENT_WIDTH, FIGURE_HEIGHT))
    plt.plot(time, cut_ecg_signal)
    plt.title('Segment ECG Signal')
    plt.xlabel('Time (seconds)')
    plt.ylabel('Amplitude')
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    if PLOT_FULL_SIGNAL:
        ...
        plot_ecg_signal(ecg_signal, SAMPLING_RATE, 
                       f"ECG Signal (ID: {ecg_id}, Sample #{SAMPLE_INDEX})")
    
    if PLOT_SEGMENT:
        print("Plotting ECG segment...")
        plot_ecg_segment(ecg_signal, SAMPLING_RATE)
    ...
    # Find the longest feature name for proper alignment
